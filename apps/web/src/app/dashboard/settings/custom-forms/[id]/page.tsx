...
const TYPES_WITH_OPTIONS = new Set<FieldType>(['select', 'multiselect', 'radio']);
...
function InlineCreateField({
  ...
  const [options, setOptions] = useState<Array<{ label: string; value: string }>>([{ label: '', value: '' }]);
  ...
  const updateOption = (idx: number, field: 'label' | 'value', val: string) => {
    setOptions(prev => prev.map((o, i) => {
      if (i !== idx) return o;
      if (field === 'label') {
        // Auto-generate value from label if value was empty or was auto-generated
        const autoValue = generateKey(val);
        const wasAutoGenerated = o.value === '' || o.value === generateKey(o.label);
        return { label: val, value: wasAutoGenerated ? autoValue : o.value };
      }
      return { ...o, [field]: val };
    }));
  };
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/v1/field-definitions', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error((data as { error?: string }).error ?? 'Failed to create field');
      }

      const created: FieldDefinition = await res.json();
      onCreated(created);
    } catch (err) {
  ...
  return (
    <div style={{ padding: '16px 20px' }}>
      <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--text-primary)', marginBottom: 12 }}>Create New Field</div>

      {error && (
        <div style={{ padding: '8px 12px', marginBottom: 12, backgroundColor: 'var(--badge-red-bg-subtle, #fef2f2)', border: '1px solid #fecaca', borderRadius: 6, color: '#dc2626', fontSize: 13 }}>
          {error}
        </div>
      )}
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <input
          ...
          value={key}
          readOnly
          style={{ ...inputStyle, backgroundColor: 'var(--bg-secondary)', color: 'var(--text-muted)', fontFamily: 'monospace', fontSize: 13 }}
        />
      </div>
      ...
      {showOptions && (
        <div style={{ marginBottom: 12 }}>
          ...
          {options.map((opt, idx) => (
            <div key={idx} style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
              ...
              <button
                ...
                style={{ background: 'none', border: 'none', cursor: options.length <= 1 ? 'not-allowed' : 'pointer', color: options.length <= 1 ? 'var(--text-muted)' : '#dc2626', padding: 4, display: 'flex', opacity: options.length <= 1 ? 0.3 : 1 }}
                title="Remove option"
              >
          ...
          >
            <Icon path={mdiPlus} size={0.5} color="currentColor" />
            Add Option
          </button>
        </div>
      )}
      ...
      {/* Actions */}
      <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end', marginTop: 16, borderTop: '1px solid var(--border-secondary)', paddingTop: 12 }}>
        <button onClick={onCancel} style={btnOutline}>Cancel</button>
        <button
          onClick={() => void handleCreate()}
          disabled={creating || !label.trim()}
          style={{ ...btnPrimary, opacity: creating || !label.trim() ? 0.6 : 1 }}
        >
...
function FieldPickerModal({
  ...
  const handleCreated = (field: FieldDefinition) => {
    // Invalidate the field definitions cache so the library refreshes
    void qc.invalidateQueries({ queryKey: ['field-definitions-picker'] });
    // Add the newly created field to the section and close
    onSelect(field);
    onClose();
  };
      ...
      <div style={{ backgroundColor: 'var(--bg-primary)', borderRadius: 12, width: '100%', maxWidth: 520, overflow: 'hidden', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border-primary)', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <h2 style={{ margin: 0, fontSize: 16, fontWeight: 700, color: 'var(--text-primary)' }}>
            {mode === 'library' ? 'Add Field from Library' : 'Create New Field'}
          </h2>
        ...
        <div style={{ display: 'flex', borderBottom: '1px solid var(--border-secondary)' }}>
          <button
            ...
            style={{
              ...
              borderBottom: mode === 'library' ? '2px solid var(--accent-primary)' : '2px solid transparent',
              cursor: 'pointer', fontSize: 13, fontWeight: mode === 'library' ? 600 : 400,
              color: mode === 'library' ? 'var(--accent-primary)' : 'var(--text-muted)',
          ...
          <button
            ...
            style={{
              flex: 1, padding: '8px 0', border: 'none',
              borderBottom: mode === 'create' ? '2px solid var(--accent-primary)' : '2px solid transparent',
              cursor: 'pointer', fontSize: 13, fontWeight: mode === 'create' ? 600 : 400,
              color: mode === 'create' ? 'var(--accent-primary)' : 'var(--text-muted)',
              backgroundColor: 'transparent',
            }}
          >
            Create New
            ...
            <div style={{ overflow: 'auto', flex: 1 }}>
              {isLoading ? (
                ...
                <div style={{ padding: 32, textAlign: 'center', color: 'var(--text-muted)' }}>
                  {search ? 'No fields match your search.' : (
                    <>
                      No active fields in the library.{' '}
                      <button onClick={() => setMode('create')} style={{ background: 'none', border: 'none', color: 'var(--accent-primary)', cursor: 'pointer', fontWeight: 600, fontSize: 13, padding: 0 }}>
                        Create one now
                      </button>
                    </>
                  )}
