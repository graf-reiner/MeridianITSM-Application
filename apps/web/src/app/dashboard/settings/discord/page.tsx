...
import { mdiArrowLeft, mdiRobot, mdiCheckCircle, mdiAlertCircle, mdiSend, mdiLoading } from '@mdi/js';
...
export default function DiscordSettingsPage() {
  ...
  const fetchChannels = async () => {
    try {
      ...
      if (res.ok) {
        ...
        setChannels(all.filter((c) => c.channelType === 'DISCORD'));
      }
    } catch { /* ignore */ }
  ...
  const handleSave = async () => {
    ...
    if (!webhookUrl.startsWith('https://discord.com/api/webhooks/')) { setMessage({ type: 'error', text: 'URL must start with https://discord.com/api/webhooks/' }); return; }
  ...
  const handleTest = async (id: string) => {
    ...
    try {
      ...
      setMessage(res.ok ? { type: 'success', text: 'Test message sent to Discord!' } : { type: 'error', text: 'Test failed' });
    ...
    <div style={{ maxWidth: 640, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 24 }}>
        ...
        <div>
          ...
          <p style={{ margin: 0, fontSize: 14, color: 'var(--text-muted)' }}>Send notifications to Discord via webhooks</p>
        </div>
      </div>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          {channels.map((ch) => (
            <div key={ch.id} style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', border: '1px solid var(--border-primary)', borderRadius: 8, marginBottom: 8 }}>
              <Icon path={mdiRobot} size={0.8} color="#5865f2" />
              <div style={{ flex: 1 }}>
      ...
      <div style={{ backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-primary)', borderRadius: 12, padding: 24 }}>
        <h2 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 600, color: 'var(--text-primary)' }}>{editId ? 'Edit Channel' : 'Add Discord Channel'}</h2>
        <div style={{ marginBottom: 14 }}>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4 }}>Channel Name</label>
          <input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., #alerts" style={inputStyle} />
        </div>
        <div style={{ marginBottom: 16 }}>
          <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', marginBottom: 4 }}>Webhook URL</label>
          <input value={webhookUrl} onChange={(e) => setWebhookUrl(e.target.value)} placeholder="https://discord.com/api/webhooks/..." style={inputStyle} />
          <p style={{ margin: '6px 0 0', fontSize: 12, color: 'var(--text-muted)' }}>
            In Discord, go to Channel Settings &rarr; Integrations &rarr; Webhooks &rarr; New Webhook &rarr; Copy Webhook URL.
          </p>
        </div>
        <div style={{ display: 'flex', gap: 8 }}>
          ...
          {editId && <button onClick={() => { setEditId(null); setName(''); setWebhookUrl(''); }} style={{ padding: '9px 18px', borderRadius: 8, border: '1px solid var(--border-primary)', backgroundColor: 'transparent', color: 'var(--text-secondary)', fontSize: 14, cursor: 'pointer' }}>Cancel</button>}
        </div>
      </div>
    </div>
  );
