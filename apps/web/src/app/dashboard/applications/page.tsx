...
const STATUS_COLORS: Record<string, { bg: string; text: string; dot: string }> = {
  ACTIVE:         { bg: 'var(--badge-green-bg-subtle)', text: '#065f46', dot: '#22c55e' },
  INACTIVE:       { bg: 'var(--bg-secondary)', text: '#6b7280', dot: '#9ca3af' },
  IN_DEVELOPMENT: { bg: 'var(--badge-blue-bg-subtle)', text: '#1e40af', dot: '#3b82f6' },
  DEPRECATED:     { bg: 'var(--badge-fuchsia-bg)', text: '#7e22ce', dot: '#a855f7' },
  DECOMMISSIONED: { bg: 'var(--bg-secondary)', text: '#9ca3af', dot: 'var(--border-secondary)' },
};
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--text-primary)', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1 }}>{data.name}</span>
      </div>
...
function DependencyGraph({ appGraphData }: { appGraphData: { nodes: AppNodeApi[]; edges: AppEdgeApi[] } | undefined }) {
  ...
  if (!appGraphData || appGraphData.nodes.length === 0) {
    return (
      <div style={{ height: 400, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--bg-secondary)', borderRadius: 10, border: '1px solid var(--border-primary)' }}>
  ...
  return (
    <>
      ...
      <div style={{ height: 420, border: '1px solid var(--border-primary)', borderRadius: 10, overflow: 'hidden' }}>
        <ReactFlow
          nodes={layoutNodes}
          edges={layoutEdges}
          ...
          maxZoom={2}
        >
          <Background gap={16} color="var(--bg-tertiary)" />
          <Controls />
        </ReactFlow>
      </div>
    </>
...
function StatCard({ label, value, icon, bg, color }: { label: string; value: string | number; icon: string; bg: string; color: string }) {
  return (
    <div style={{ backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-primary)', borderRadius: 10, padding: '16px 18px', display: 'flex', alignItems: 'center', gap: 14 }}>
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 24, flexWrap: 'wrap' }}>
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 8, flex: 1 }}>
          <Icon path={mdiApplicationCog} size={1.1} color="var(--accent-primary)" />
          Application Portfolio
        </h1>
        <button
          onClick={() => void refetch()}
          style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '7px 14px', border: '1px solid var(--border-secondary)', borderRadius: 7, fontSize: 13, cursor: 'pointer', backgroundColor: 'var(--bg-primary)', color: 'var(--text-secondary)' }}
        >
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: 12, marginBottom: 28 }}>
        {statsLoading ? (
          Array.from({ length: 4 }).map((_, i) => (
            <div key={i} style={{ backgroundColor: 'var(--bg-secondary)', border: '1px solid var(--border-primary)', borderRadius: 10, height: 76 }} />
          ))
        ) : (
          <>
            <StatCard
      ...
      <div style={{ backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-primary)', borderRadius: 12, padding: 20, marginBottom: 28 }}>
        <h2 style={{ margin: '0 0 14px', fontSize: 16, fontWeight: 700, color: 'var(--text-primary)' }}>Application Dependency Diagram</h2>
        {graphLoading ? (
          <div style={{ height: 420, display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--bg-secondary)', borderRadius: 10 }}>
            <span style={{ color: 'var(--text-placeholder)', fontSize: 14 }}>Loading dependency graph...</span>
          </div>
        ) : (
          <ReactFlowProvider>
        ...
        <p style={{ margin: '10px 0 0', fontSize: 12, color: 'var(--text-placeholder)' }}>
      ...
      <div style={{ backgroundColor: 'var(--bg-primary)', border: '1px solid var(--border-primary)', borderRadius: 12, overflow: 'hidden' }}>
        {/* Table header */}
        <div style={{ padding: '14px 18px', borderBottom: '1px solid var(--border-primary)', display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <h2 style={{ margin: 0, fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', flex: 1 }}>
            Application Matrix
            {listData && <span style={{ marginLeft: 8, fontSize: 13, fontWeight: 400, color: 'var(--text-muted)' }}>({listData.total} total)</span>}
          </h2>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            ...
            <select
              ...
              style={{ padding: '5px 10px', border: '1px solid var(--border-secondary)', borderRadius: 6, fontSize: 12, outline: 'none' }}
            >
        ...
        {listLoading ? (
          <div style={{ padding: 40, textAlign: 'center', color: 'var(--text-placeholder)' }}>Loading applications...</div>
        ) : (
          <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
            <thead>
              <tr style={{ backgroundColor: 'var(--bg-secondary)', borderBottom: '2px solid var(--border-primary)' }}>
                <th style={{ padding: '9px 14px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Name</th>
                <th style={{ padding: '9px 14px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Type</th>
                <th style={{ padding: '9px 14px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Status</th>
                <th style={{ padding: '9px 14px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Criticality</th>
                <th style={{ padding: '9px 14px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Hosting</th>
                <th style={{ padding: '9px 14px', textAlign: 'left', fontWeight: 600, color: 'var(--text-secondary)' }}>Lifecycle</th>
                <th style={{ padding: '9px 14px', textAlign: 'right', fontWeight: 600, color: 'var(--text-secondary)' }}>Annual Cost</th>
              </tr>
            </thead>
            <tbody>
              {applications.map((app) => {
                ...
                return (
                  ...
                  >
                    <td style={{ padding: '9px 14px', fontWeight: 600, color: 'var(--text-primary)' }}>{app.name}</td>
                    <td style={{ padding: '9px 14px', color: 'var(--text-muted)' }}>{app.type}</td>
                    ...
                    <td style={{ padding: '9px 14px', color: 'var(--text-muted)' }}>{app.hostingModel ?? '—'}</td>
                    <td style={{ padding: '9px 14px', color: 'var(--text-muted)' }}>{app.lifecycleStage ?? '—'}</td>
              ...
              {applications.length === 0 && (
                <tr>
                  <td colSpan={7} style={{ padding: 32, textAlign: 'center', color: 'var(--text-placeholder)' }}>No applications found</td>
                </tr>
              )}
            </tbody>
          </table>
