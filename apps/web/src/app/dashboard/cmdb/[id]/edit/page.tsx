...
interface GroupOption {
  id: string;
  name: string;
  isCmdbSupportGroup?: boolean;
}

interface CIDetailFull {
  id: string;
  ciNumber: string;
  name: string;
  displayName?: string;
  ...
  networkExt?: Record<string, unknown>;
  cloudExt?: Record<string, unknown>;
  endpointExt?: Record<string, unknown>;
  serviceExt?: Record<string, unknown>;
}
...
// Edit page has 4 steps (no class selection — class is locked)
const STEPS = [
  { label: 'General Info', num: 1 },
  { label: 'Ownership', num: 2 },
  { label: 'Technical Details', num: 3 },
  { label: 'Review & Save', num: 4 },
];
...
export default function CMDBEditPage() {
  ...
  useEffect(() => {
    ...
    void fetchJson('/api/v1/settings/groups').then((d) => {
      const all = d.data ?? d ?? [];
      setGroups(all.filter((g: GroupOption) => g.isCmdbSupportGroup));
    });
    void fetchJson('/api/v1/settings/users?pageSize=200').then((d) => setUsers(d.data ?? d.users ?? d ?? []));
    ...
    void (async () => {
      try {
        ...
        const ci = await res.json() as CIDetailFull;
        populateForm(ci);
      } catch (err) {
        setLoadError(err instanceof Error ? err.message : 'Failed to load CI');
      } finally {
        setLoading(false);
      }
    })();
  ...
  function populateForm(ci: CIDetailFull) {
    ...
    // General
    setName(ci.name ?? '');
    setDisplayName(ci.displayName ?? '');
    setLifecycleStatusId(ci.lifecycleStatusId ?? '');
    setOperationalStatusId(ci.operationalStatusId ?? '');
    setEnvironmentId(ci.environmentId ?? '');
    setCriticality(ci.criticality ?? '');
    setDescription(ci.description ?? '');

    // Ownership
    setBusinessOwnerId(ci.businessOwnerId ?? '');
    setTechnicalOwnerId(ci.technicalOwnerId ?? '');
    ...
    setFqdn(ci.fqdn ?? '');
    setIpAddress(ci.ipAddress ?? '');
    setSerialNumber(ci.serialNumber ?? '');
    setAssetTag(ci.assetTag ?? '');
    setExternalId(ci.externalId ?? '');
    setModel(ci.model ?? '');
    setVersion(ci.version ?? '');
    setEdition(ci.edition ?? '');

    // Server extension
    if (ci.serverExt) {
      const ext = ci.serverExt;
      setServerType(String(ext.serverType ?? ''));
      setOperatingSystem(String(ext.operatingSystem ?? ''));
      setOsVersion(String(ext.osVersion ?? ''));
      setCpuCount(ext.cpuCount != null ? String(ext.cpuCount) : '');
      setMemoryGb(ext.memoryGb != null ? String(ext.memoryGb) : '');
      setStorageGb(ext.storageGb != null ? String(ext.storageGb) : '');
      setBackupRequired(!!ext.backupRequired);
      setBackupPolicy(String(ext.backupPolicy ?? ''));
      setPatchGroup(String(ext.patchGroup ?? ''));
    }

    // Application extension
    if (ci.applicationExt) {
      const ext = ci.applicationExt;
      setApplicationType(String(ext.applicationType ?? ''));
      setInstallType(String(ext.installType ?? ''));
      setBusinessFunction(String(ext.businessFunction ?? ''));
      setRepoUrl(String(ext.repoUrl ?? ''));
      setDocumentationUrl(String(ext.documentationUrl ?? ''));
      setPrimaryLanguage(String(ext.primaryLanguage ?? ''));
      setRuntimePlatform(String(ext.runtimePlatform ?? ''));
      setAuthenticationMethod(String(ext.authenticationMethod ?? ''));
      setInternetFacing(!!ext.internetFacing);
      setComplianceScope(String(ext.complianceScope ?? ''));
    }

    // Database extension
    if (ci.databaseExt) {
      const ext = ci.databaseExt;
      setDbEngine(String(ext.dbEngine ?? ''));
      setDbVersion(String(ext.dbVersion ?? ''));
      setInstanceName(String(ext.instanceName ?? ''));
      setDbPort(ext.port != null ? String(ext.port) : '');
      setDbBackupRequired(!!ext.backupRequired);
      setBackupFrequency(String(ext.backupFrequency ?? ''));
      setEncryptionEnabled(!!ext.encryptionEnabled);
      setContainsSensitiveData(!!ext.containsSensitiveData);
    }

    // Network extension
    if (ci.networkExt) {
      const ext = ci.networkExt;
      setDeviceType(String(ext.deviceType ?? ''));
      setFirmwareVersion(String(ext.firmwareVersion ?? ''));
      setManagementIp(String(ext.managementIp ?? ''));
      setMacAddress(String(ext.macAddress ?? ''));
      setRackLocation(String(ext.rackLocation ?? ''));
      setHaRole(String(ext.haRole ?? ''));
    }

    // Cloud extension
    if (ci.cloudExt) {
      const ext = ci.cloudExt;
      setCloudProvider(String(ext.cloudProvider ?? ''));
      setRegion(String(ext.region ?? ''));
      setResourceGroup(String(ext.resourceGroup ?? ''));
      setResourceType(String(ext.resourceType ?? ''));
      setNativeResourceId(String(ext.nativeResourceId ?? ''));
    }

    // Endpoint extension
    if (ci.endpointExt) {
      const ext = ci.endpointExt;
      setEndpointType(String(ext.endpointType ?? ''));
      setProtocol(String(ext.protocol ?? ''));
      setPort(ext.port != null ? String(ext.port) : '');
      setUrl(String(ext.url ?? ''));
      setDnsName(String(ext.dnsName ?? ''));
      setCertificateExpiryDate(String(ext.certificateExpiryDate ?? ''));
      setCertificateIssuer(String(ext.certificateIssuer ?? ''));
      setTlsRequired(!!ext.tlsRequired);
    }

    // Service extension
    if (ci.serviceExt) {
      const ext = ci.serviceExt;
      setServiceType(String(ext.serviceType ?? ''));
      setServiceTier(String(ext.serviceTier ?? ''));
      setSlaName(String(ext.slaName ?? ''));
      setAvailabilityTarget(ext.availabilityTarget != null ? String(ext.availabilityTarget) : '');
      setRtoMinutes(ext.rtoMinutes != null ? String(ext.rtoMinutes) : '');
      setRpoMinutes(ext.rpoMinutes != null ? String(ext.rpoMinutes) : '');
    }
  }
  ...
  async function handleSubmit() {
    ...
    setError(null);
    try {
      const res = await fetch(`/api/v1/cmdb/cis/${id}`, {
        method: 'PUT',
        credentials: 'include',
      ...
      if (!res.ok) {
        ...
        throw new Error((body as { error?: string }).error || `Failed to update CI: ${res.status}`);
      }
      router.push(`/dashboard/cmdb/${id}`);
    } catch (err) {
  ...
  function renderStep1() {
    return (
      <div style={cardStyle}>
        {/* Show locked class */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 20, padding: '10px 14px', backgroundColor: 'var(--bg-secondary)', borderRadius: 8, border: '1px solid var(--border-primary)' }}>
          <Icon path={getClassIcon(classSlug)} size={1} color="var(--accent-primary)" />
          <div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', fontWeight: 500 }}>CI Class (read-only)</div>
            <div style={{ fontSize: 15, fontWeight: 600, color: 'var(--text-primary)' }}>{className || 'Unknown'}</div>
          </div>
        </div>

        <h2 style={{ margin: '0 0 4px', fontSize: 16, fontWeight: 600, color: 'var(--text-primary)' }}>General Information</h2>
        <p style={{ margin: '0 0 20px', fontSize: 13, color: 'var(--text-muted)' }}>Update the basic information for this configuration item.</p>
  ...
  function renderStep4() {
    ...
    const ownerLabel = (oid: string) => {
      const u = users.find((u) => u.id === oid);
      return u ? `${u.firstName} ${u.lastName}` : oid;
    };
    ...
    return (
      <div>
        <div style={{ ...cardStyle, marginBottom: 16 }}>
          ...
          <p style={{ margin: '0 0 8px', fontSize: 13, color: 'var(--text-muted)' }}>Review the updated configuration item details before saving.</p>
        </div>
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <h1 style={{ margin: 0, fontSize: 22, fontWeight: 700, color: 'var(--text-primary)', display: 'flex', alignItems: 'center', gap: 8 }}>
          <Icon path={mdiPencil} size={1} color="var(--accent-primary)" />
